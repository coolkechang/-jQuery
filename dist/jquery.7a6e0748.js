...
window.$ = window.jQuery = function (selectorOrArray) {
  ...
  return {
    ...
    find: function find(selector) {
      ...
      for (var i = 0; i < elements.length; i++) {
        var element2 = Array.from(elements[i].querySelectorAll(selector));
        array = array.concat(element2); //用之前的空数组连接上新的元素，把连接上新元素的数组放回到array。（得到的是伪数组，需要用Array.from转化成数组再放回）
      }
    ...
    children: function children() {
      var array = [];
      this.each(function (node) {
        array.push.apply(array, _toConsumableArray(node.children)); //...是指展开操作符，就是把数组展开作为第一二三...个参数展示
      });
      return jQuery(array);
    },
